...
- **`kajabi-thank-you-page-embed.html`** — the snippet to paste into a Custom Code block on the Kajabi thank-you page. It's the front-door bridge: Kajabi's "custom thank you page" setting only accepts a Kajabi page from a dropdown (never an external URL), so contact details land there first and this forwards them to the Netlify-hosted quiz in an iframe, plus listens for the widget's `quizHeight` messages. Not used by anything in this repo — it lives in Kajabi.
