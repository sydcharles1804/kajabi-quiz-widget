...
The two products are written **`SCHOLAR Prep™`** and **`SCHOLAR Score™`** — `SCHOLAR` always fully capitalized, and a trademark symbol after *every* occurrence in visitor-facing copy, not just the first use on a page. This holds across the quiz screen copy, the emailed report, and the SCHOLAR Prep™ landing page.

- In HTML (`quiz-widget.html`, the two landing-page snippets) use the entity **`&trade;`**, never a literal ™. The report is a fragment merged into Kajabi's email template, and the entity survives every client and charset guess.
- In the Markdown/txt content specs use the literal **™**.
- Do **not** mark other `SCHOLAR`-prefixed phrases from this rule: `SCHOLAR Profile`, `Scholarship Categories`, `Scholars Get Dollars`, `College and Scholarship Team`. The one exception is the landing page's "Assess your child's **SCHOLAR Profile™**" bullet, which carries a mark in the source copy.
- **The mark always follows the complete term** — `SCHOLAR Prep™`, `SCHOLAR Score™`, `SCHOLAR Profile™`. Never split it as `SCHOLAR™ Prep` or `SCHOLAR™ Profile`.
- Never touch the lowercase code forms: the `scholarPrep`/`scholarsGetDollars` keys in `OFFER_URLS`, the `/scholar-prep` and `/scholars-get-dollars` slugs, or the `id="scholar-prep-page"` hook. The `/scholar prep/i` test in `buildReportText()` that picks the offer URL matches case-insensitively on the offer heading, so it keeps working with `&trade;` appended.
...
- **Kajabi's redirect drops `?email=` in real browsers — don't trust it alone.** Diagnosed 2026-08-07 against form `2149687846` on `/sgd`. Established by testing: `/forms/<id>/thank_you` only *forwards* a query string, it never *injects* one (`thank_you?email=x` → `/scholar-score-diagnostic?email=x`, but `thank_you` bare → `/scholar-score-diagnostic` bare). Server-side the parameter is present at every hop, yet a real browser lands on the quiz page with no query string at all, so the loss happens somewhere in Kajabi's Turbolinks submit path and is not reproducible with curl. The fix is the `localStorage` fallback described above, not more Kajabi configuration — the settings were verified correct. **The capture must read the field on `input`/`change`, never on `submit`:** Kajabi intercepts submission with its own JavaScript (Parsley plus a Turbolinks AJAX post), so a `submit` listener added from a Custom Code block never fires — a first attempt that hooked `submit` looked correct and silently did nothing. Confirmed working 2026-08-07: a real funnel run saved the visitor's address instead of `null`. Note also that a **template update silently replaces the form on a page with a brand-new, unconfigured form object** (the old form here was `2149185801`); if the funnel breaks after a template change, check the form ID in the page's HTML before assuming a settings problem.
